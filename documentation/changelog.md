...
### v3.3.0-dev.37 — 2026-09-22 — Discussion files renamed to *.chippy.md; system files to *.sys.chippy.md

> Every file Chippy owns now ends in `.chippy.md`, so one glob selects them all and a data folder can hold other Markdown without it being mistaken for a discussion. Discussions are `<stem>.chippy.md`, archived ones `<stem>.archive.chippy.md`, and the four app-managed files move out of the plain `.chippy.md` namespace to `navigation.sys.chippy.md`, `tags.sys.chippy.md`, `names.sys.chippy.md`, `summary.sys.chippy.md`. The kind of a file is decided by the segment before `.chippy.md` — a stem never contains a dot (sanitizeName strips them), so the namespaces cannot collide and `tags.chippy.md` is simply a discussion called "tags". Existing folders migrate on first open, automatically and in a fixed order: the system files first (gen-3 `*.chippy.md` → `*.sys.chippy.md`, byte-identical renames; the pre-v3.1 layouts go straight to the `.sys` names), then a sweep renames every `<stem>.md` / `<stem>.archive.md` into the new layout — never overwriting an existing target, skipping (and retrying next open) files that cannot be read, leaving non-Markdown and never-loadable names (`foo.bar.md`) untouched, and logging the renamed list. A partially migrated folder stays fully usable: a pending `<stem>.md` is still listed and loaded, and its next save writes the new name and removes the old file. **Visible change for users:** the first open of an existing folder renames every discussion file — anything outside Chippy that points at `<stem>.md` (scripts, editor bookmarks, the dev-task-runner `agent.md`) must be updated to `<stem>.chippy.md`.

- **`io.js`** — new constants `CHIPPY_SUFFIX` / `ARCHIVE_SUFFIX` / `SYS_SUFFIX`, system filenames `*.sys.chippy.md`, gen-3 constants for the old `*.chippy.md` system names; `classifyFilename` (discussion / archive / sys / legacy-discussion / legacy-archive / other), `discussionFilename`, `archiveFilename`; `listDiscussions` / `listArchived` classify instead of suffix-matching and include pending legacy files; `loadDiscussion` resolves the current file with a legacy fallback, `saveDiscussion` writes `.chippy.md` and removes a surviving `<stem>.md`, `archiveDiscussion` / `renameDiscussion` operate on the resolved file (rename also refuses a target whose legacy file still exists); new `migrateDiscussionFilenames` sweep and `migrateGen2Indexes`; `loadIndexes` chains sys → gen-3 → legacy and always runs the sweep last.
- **`format.js`** — `parseDiscussion` filename fallback strips `.archive.chippy.md` / `.chippy.md` / `.md`.
- **`store.js`**, **`discussion.js`**, **`main.js`**, **`pages.js`** — rename-collision message uses `io.discussionFilename`; archive confirm and help texts say `*.archive.chippy.md`; the Summary-settings help line names `summary.sys.chippy.md` (was a stale `summary.md`).
- **`documentation/datadefinition.md`** — intro file-kind table, §1 naming rules, new §1.1 filename sweep, §3 renamed to system files, §3.4 gains generation 3 and the migration order, §4 and §5 tree updated.

  ...
  function showHelp() {
    ...
    ui.showModal('Chippy — Help', (modal, close) => {
      ...
      section('Page overview', 'Sidebar buttons open these cross-discussion pages, each with the unified #tag / @name / free-text search. Double-click any comment card (Comments, Tasks, Goals, Ideas, Ro3, Calendar) to jump to that exact comment in its discussion.', [
        ...
        ['Activity', 'charts: comment inflow (incl. ideas), task/goal/idea states, monthly timeline, tasks-over-time state areas (open/WIP/CHK/HOLD/PRGT, rebuilt from the dated state-change actions), and tasks-created-per-week by state.'],

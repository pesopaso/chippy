...
Thirteen **cross-discussion views** are accessible from sidebar navigation buttons: Tags (tag overview with staleness coloring), Comments (all comments), Tasks (all open tasks as list), Kanban (drag-and-drop board by state), Goals (all open goals), Ideas (all ideas with lifecycle state tabs), Images (all images), Links (all links aggregated from entries and prep notes with inline title editing), **Names (registered `@[Name]` references with mention counts, last-seen dates, and per-name drill-down)**, Calendar (due-date calendar over open tasks/followups with day/focus/work/full/month views), Ro3 (Rule of Three — randomly selects three open tasks, with discussion tag filtering), Activity (dashboard with pie charts for entry types, task/goal/idea states, a monthly timeline, burndown and per-day execution charts), and AI Summary (consolidates a discussion history via a configurable LLM endpoint, with sensitive content excluded). Double-clicking a comment card on Comments, Tasks, Goals, Ideas, Ro3, or Calendar jumps to that exact comment in its discussion. Each list view exposes a single search box that accepts `#tag` to filter by entry tag, `@Name` or `@[Full Name]` to filter by name reference, and any remaining text as freetext substring match.
...
| 60 | Create discussion                 | A `+` button in the sidebar header (visible only when a folder is open, aligned to the right of the "Discussions" label) creates a new empty discussion named "undefined". The discussion file is written to disk immediately and registered in `navigation.md`. The button navigates to the new discussion so the user can rename it. If a discussion named "undefined" already exists, a numeric suffix is appended (`undefined_2`, `undefined_3`, …). |
| 61 | Discussion tag grouping in sidebar | Discussions in the sidebar are grouped under colored section headers by their `tag` field (from `navigation.md`). Each tag is assigned a stable color from a fixed palette by sorted index. Within each group, discussions render favorites-first then alphabetically. Discussions without a tag render last under an "Untagged" header. A row of colored filter buttons above the discussion list lets the user restrict the sidebar to a single group; clicking the active button clears the filter. When the active filter's tag no longer exists it auto-resets. Infrastructure: `getDiscussionTags()` selector (sorted, deduplicated), `getDiscTagColor(tag, allTags)` pure helper in `ui.js`, `setDiscussionTag(name, tag)` store action (persists to `navigation.md`, emits `discussionTagChanged`). |
| 62 | Inline discussion tag editor      | The discussion view shows a tag editor beneath the discussion title. When no tag is set it renders a text input with a `<datalist>` of existing discussion tags for autocomplete; on commit it calls `store.setDiscussionTag`. When a tag is set it renders a colored chip (matching the sidebar color) with a `×` remove button that clears the tag. Tag changes cause the sidebar to re-group immediately via the `discussionTagChanged` subscriber. |
| 63 | Discussion tag filter on cross-discussion pages | Every cross-discussion list page (All Comments, All Tasks, Kanban, All Goals, All Images, All Links, All Names) shows a row of colored discussion-tag filter buttons rendered by a shared `renderDiscTagFilterButtons(opts)` helper in `ui.js`. Each page maintains its own local filter state (reset to "All" on page entry). The filter narrows results to discussions carrying that tag by passing `discTag` to `store.collectEntries` (or an equivalent guard). The Kanban and All Tasks pages share a single filter state. Export filenames on All Tasks and All Goals encode the active filter. |
| 64 | Discussion rename                 | Pencil icon next to the discussion title renames the discussion in place: the `.md` file and image subfolder are renamed, image references inside entries are rewritten, and navigation/cache/recent entries are updated (added v3.0.0). |
| 65 | Task links (multi-discussion tasks) | A task, followup, or idea can be connected (🔗) to other discussions and appears in each — in the history at the date it was connected and in the Open Tasks/Ideas panel. The entry lives once in its origin discussion; connected discussions store a reference carrying the shared `<origin-stem>:link-<id>` tag that resolves to the live origin at render time (single source of truth: state, priority, due, text, and actions written from anywhere are saved at the origin). Cross-views count a linked task once. ✂ disconnects (removes only that discussion's reference); a reference whose origin is gone renders as a read-only ⚠ stub in the history and leaves the panels; renaming a discussion rewrites the link-tag stems everywhere (see `datadefinition.md` §2.4 and `task-linking-implementation.md`). |
| 66 | Sensitive markers                 | A lock button (🔓 ⇄ 🔒) on each comment (discussion stream only) and next to the discussion title marks a comment or a whole discussion sensitive; marked content shows a red "sensitive" chip and a red card border. Sensitive content is excluded from automatically created AI summaries — the prompt sent to the LLM never contains it. Stored as the reserved `sensitive` entry tag and the `| sensitive` navigation flag. Stream ergonomics shipped alongside: mute appears only on the right-hand task/followup rows, and the stream's delete button is set apart and ~20% larger. |

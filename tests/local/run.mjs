...
// A discussion file is a root-level <stem>.chippy.md that is neither an
// archived discussion nor a *.sys.chippy.md system file (datadefinition §1).
function seedHasData() {
  if (!existsSync(SEED)) return false;
  return readdirSync(SEED).some(
    f => f.endsWith('.chippy.md') && !f.endsWith('.archive.chippy.md') && !f.endsWith('.sys.chippy.md')
  );
}
